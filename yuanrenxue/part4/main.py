...
def base64_map_number(base64_img : str) -> int :
    """
    通过base64图片找到对应的数字

    Args:
        base64_img (str): base64图片

    Returns:
        int: 对应的数字
    """
    number = number_base[base64_img]
    return number


def data_sort_by_left(data, lefts) -> int:
    """
    根据left值为base64图片排序
    ...
    base_px = 11.5
    result = 0
    num_list = [[] for _ in range(len(lefts))]  #为方便排序，根据left长度预先创建一个二维数组，其中每个数组存放一个数字

    for i, left in enumerate(lefts):
        l = float(left[5:-2])    #剪去头尾，只要中间的数字  ：：left: -11.5px -> -11.5
        index = int(i + (l/base_px))    #计算偏移量 i为当前的位置 + (l/base_px)等于偏移的距离 ：：-11.5/11.5 = -1， -23/11.5 = -2， 11.5/11.5 = 1， 0/11.5 = 0 

        print(l, i, index)
        
        num = str(base64_map_number(data[i]))
        num_list[index] = num
...
for i in range(1,6):
    ...
    response = requests.get('https://match.yuanrenxue.cn/api/match/4', params=params, cookies=cookies, headers=headers).json()

    value = response['value']
    key = response['key']
    j_key = md5(b64encode((key + value).encode()).decode().replace('=', '').encode()).hexdigest()
    base64_img = parse_spritemap(response['info'], j_key)

    for i in base64_img:
        num_sum += data_sort_by_left(i[0], i[1])
